...
const lib = new fastcall.Library("../build/libabieos.so")
    ...
    .function(
        "int abieos_set_abi_hex(void* context, uint64 contract, char* hex)"
    )
    .function(
...
function check_types() {
    ...
    check_throw("Error: Unsupported abi version", () =>
        eosjs_hex_abi_to_json(
            eosjs_json_abi_to_hex({ version: "eosio::abi/9.0" })
        )
    );
    ...
    check_type(
        ...
        js2Types,
        "block_timestamp_type",
        '"2018-06-15T19:17:47.500"'
    );
    check_type(
        0,
        js2Types,
        "block_timestamp_type",
        '"2018-06-15T19:17:48.000"'
    );
    check_throw("Error: Invalid time format", () =>
...
async function push_transfer() {
    if (useTokenHexApi)
        ...
        check(
            l.abieos_set_abi(
                context,
                name("eosio.token"),
                jsonStr((await rpc.get_abi("eosio.token")).abi)
            )
        );
    let type = checkPtr(
    ...
    console.log(
        "action bin->json: ",
        abieos_hex_to_json(name("eosio.token"), "transfer", actionDataHex)
    );
